...
public:
    ...
    bool searchCar(string brand, Node* root, string price, string model, string mileage);
...
bool BST::searchCar(string brand, Node* root, string price, string model, string mileage)
{
    ...
    else
    {
        ...
        if (root->brand == brand && stoi(root->price) <= stoi(price) && root->model == model && stoi(root->mileage) <= stoi(mileage))
        {
            ...
            //cout << endl;
            searchCar(brand, root->left, price, model, mileage);
            searchCar(brand, root->right, price, model, mileage);
        }
...
bool AVLTree::searchCar(string brand, Node* root, string price, string model, string mileage)
{
    // If tree is empty, print unsuccessful
    if (this->root == nullptr)
    {
...
int main(int argc, char *argv[])
{
    ...
    getline(file, k, ',');
    getline(file, l);
    z.setRoot(z.insertdataID(nullptr, c, d, e, f, g, h, i, j, k, l, stoi(b)));
    ...
    while (stoi(b) < 1000)
    {
        ...
        getline(file,k,',');
        getline(file,l);
        z.insertdataID(z.getRoot(), c, d, e, f, g, h, i, j, k, l, stoi(b));
    }
